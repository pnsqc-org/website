(() => {
  ...
  const fallbackAvatar = moduleRoot.getAttribute("data-speaker-fallback-avatar") || "/images/brand/pnsqc-logo.jpg";
  ...
  const normalizeCompareText = (value) =>
    normalizeSpace(value).replace(/:\s*$/, "").toLowerCase();
  ...
  const buildModalTemplate = (speaker, details, categoryName) => {
    ...
    const avatar = createEl("img", "h-20 w-20 rounded-lg object-cover ring-2 ring-pnsqc-gold/30", null);
    ...
    abstractSection.appendChild(createEl("p", "text-xs font-semibold uppercase tracking-widest text-pnsqc-gold/80", "Abstract"));
    ...
    bioSection.appendChild(createEl("p", "text-xs font-semibold uppercase tracking-widest text-pnsqc-gold/80", "Bio"));
    ...
    return { template, templateId: template.id, speakerName, presentationTitle, categoryLabel: categoryName || "Speaker" };
  };
  ...
  const buildCard = ({ speaker, details, templateId, categoryLabel }) => {
    ...
    const avatar = createEl("img", "w-32 h-32 rounded-lg object-cover ring-2 ring-pnsqc-gold/30", null);
    avatar.src = speaker.avatar || fallbackAvatar;
    avatar.alt = speakerName;
    avatar.loading = "lazy";
  ...
  fetch(endpoint)
    ...
    .then((payload) => {
      ...
      const categories = Array.isArray(eventData.speaker_categories) ? eventData.speaker_categories : [];

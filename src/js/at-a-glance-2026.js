(() => {
  ...
  class TimeUtils {
    ...
    static formatMinutesForDisplay(dayIso, minutesFromMidnight, timeFormat, displayTimeZone, eventTimeZone) {
      const utcMillis = TimeUtils.getUtcMillisForEventTime(dayIso, minutesFromMidnight, eventTimeZone);
    ...
    static formatSessionTimeRange(session, dayIso, timeFormat, displayTimeZone, eventTimeZone) {
      ...
      const startText = startMinutes !== null
        ? TimeUtils.formatMinutesForDisplay(dayIso, startMinutes, timeFormat, displayTimeZone, eventTimeZone)
        : '';
      const endText = endMinutes !== null
        ? TimeUtils.formatMinutesForDisplay(dayIso, endMinutes, timeFormat, displayTimeZone, eventTimeZone)
        : '';
  ...
  class MeetinghandClient {
    ...
    async fetchSubmissions(ids, submissionDetails) {
      if (!ids || ids.length === 0) return;
      const requests = ids.map((id) => fetch(`https://api.meetinghand.com/api/events/pnsqc-2026/submissions/${id}`, {
        ...
        .then((payload) => {
          const description = SubmissionFormatter.buildSubmissionDescription(payload);
          if (description) submissionDetails.set(String(id), description);
        })
        .catch(() => null));
  ...
  class ModalTemplateManager {
    ...
    createAbstractTemplate({ isSubmissionType, submissionDetail, abstractText }) {
      ...
      if (isSubmissionType && submissionDetail) {
        if (submissionDetail.abstract) {
          ...
          abstractSection.appendChild(Dom.el('p', 'text-xs font-semibold uppercase tracking-widest text-pnsqc-gold/80', 'Abstract'));
          const abstractBody = Dom.el('div', 'schedule-modal-content text-sm leading-7 text-pnsqc-slate space-y-3');
        ...
        if (submissionDetail.objectives) {
          ...
          objectiveSection.appendChild(Dom.el('p', 'text-xs font-semibold uppercase tracking-widest text-pnsqc-gold/80', 'Learning Objectives'));
          const objectivesBody = Dom.el('div', 'schedule-modal-content text-sm leading-7 text-pnsqc-slate space-y-3');
        ...
        wrapper.appendChild(Dom.el('p', 'text-sm leading-7 text-pnsqc-slate whitespace-pre-line', abstractText));
      }
    ...
    createSpeakerTemplate({ speakerObj, itemSpeaker, itemTitle, submissionBio, authorTitle, useSubmission }) {
      ...
      topContent.appendChild(Dom.el('h4', 'text-lg font-semibold text-white', itemSpeaker || 'Speaker'));
      ...
      if (speakerObj && !useSubmission) {
        ...
        const addIcon = (opts) => {
          if (!opts.href) return;
          const link = Dom.el('a', 'inline-flex items-center justify-center w-9 h-9 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 transition-colors');
          link.href = opts.href;
          link.target = '_blank';
      ...
      bioSection.appendChild(Dom.el('p', 'text-xs font-semibold uppercase tracking-widest text-pnsqc-gold/80', 'Bio'));

      if (useSubmission) {
        const submissionBioBody = Dom.el('div', 'schedule-modal-content text-sm leading-7 text-pnsqc-slate space-y-3');
        submissionBioBody.innerHTML = submissionBio || 'Bio coming soon.';
        bioSection.appendChild(submissionBioBody);
      } else {
        bioSection.appendChild(Dom.el('p', 'text-sm leading-7 text-pnsqc-slate whitespace-pre-line', speakerObj?.short_bio || 'Bio coming soon.'));
      }
  ...
  class ScheduleRenderer {
    constructor({ root, dayNav, trackLegend, timezoneToggle, timezoneLabel, eventMeta, eventIntro, templateRoot }) {
      this.root = root;
      this.dayNav = dayNav;
      ...
      days.forEach((day) => {
        ...
        sessions.forEach((session) => {
          ...
          items.forEach((item) => {
            ...
            if (!presentation || !presentation.presentation_type) return;
            const submissionId = item?.participant_submission_id || presentation.id;
            if (submissionId) ids.add(String(submissionId));
          });
        });
      });
    ...
    speakerFromItem(item) {
      ...
      if (presentation.participant) {
        const name = `${presentation.participant.firstname || ''} ${presentation.participant.lastname || ''}`.trim();
        if (name) return name;
      }
    ...
    renderTimezoneToggle(eventTimeZone) {
    ...
    renderDayNavigation(days) {
      ...
      if (!days.length) {
        this.dayNav.appendChild(Dom.el(
          'span',
      ...
      if (!isBreak && items.length > 0) {
        ...
        items
          ...
          .forEach((item, index) => {
            ...
            const startMinutes = index === 0 ? baseStartMinutes : (typeof lastEndMinutes === 'number' ? lastEndMinutes + 10 : null);
            ...
            const submissionId = item?.participant_submission_id ? String(item.participant_submission_id) : (presentation?.id ? String(presentation.id) : '');
            const submissionDetail = isSubmissionType && submissionId ? this.submissionDetails.get(submissionId) : null;

            let abstractText = '';
            if (isSubmissionType && submissionDetail) {
              if (submissionDetail.abstract) abstractText += `Abstract\n\n${submissionDetail.abstract}`;
              if (submissionDetail.objectives) {
                abstractText += `${abstractText ? '\n\n' : ''}Learning Objectives\n\n${submissionDetail.objectives}`;
              }
            } else {
              abstractText = abstractMap.get(TextUtils.normalizeCompareText(itemTitle)) || '';
            }

            const timeLabel = (typeof startMinutes === 'number' && typeof endMinutes === 'number')
            ...
            const row = isKeynote
              ? Dom.el('div', 'flex items-start gap-2')
              : Dom.el('div', 'grid gap-x-4 gap-y-1 sm:grid-cols-[140px,1fr]');
            if (!isKeynote) {
              row.appendChild(Dom.el('span', 'text-xs uppercase tracking-widest text-pnsqc-gold font-semibold tabular-nums leading-6', timeLabel));
            }

            const detail = Dom.el('div', 'space-y-1');
            const titleRowInner = Dom.el('div', 'flex items-center gap-2');
            ...
            if (abstractText) {
              ...
              titleRowInner.appendChild(titleButton);
            } else {
              const itemTitleClass = isKeynote ? 'text-white font-bold text-lg sm:text-xl' : 'text-white font-medium';
              titleRowInner.appendChild(Dom.el('p', itemTitleClass, itemTitle));
            }
            ...
            if (itemSpeaker) {
              if (speakerObj && !isSubmissionType) {
                ...
                detail.appendChild(speakerButton);
              } else if (isSubmissionType) {
                const submissionBio = submissionDetail?.bio || 'Bio coming soon.';
                const authorTitle = presentation?.authors?.[0]?.title ? String(presentation.authors[0].title) : '';
                ...
                const submissionSpeakerButtonClass = isKeynote
                  ? 'text-pnsqc-gold-light text-sm font-medium hover:text-pnsqc-gold transition-colors text-left'
                  : 'text-pnsqc-slate text-sm hover:text-pnsqc-gold transition-colors text-left';
                const submissionSpeakerButton = Dom.el('button', submissionSpeakerButtonClass, itemSpeaker);
                submissionSpeakerButton.type = 'button';
                submissionSpeakerButton.setAttribute('data-track-modal-open', submissionSpeakerTemplateId);
                ...
                detail.appendChild(submissionSpeakerButton);
              } else {
                const speakerLineClass = isKeynote ? 'text-pnsqc-gold-light text-sm font-medium' : 'text-pnsqc-slate text-sm';
                detail.appendChild(Dom.el('p', speakerLineClass, itemSpeaker));
              }
            }
    ...
    render(data) {
      ...
      this.root.innerHTML = '';
      if (days.length === 0) {
        const emptyCard = Dom.el('div', 'p-6 bg-pnsqc-blue/10 border border-pnsqc-gold/20 rounded-lg');
        emptyCard.appendChild(Dom.el('p', 'text-pnsqc-slate text-lg', 'Schedule details are coming soon.'));
      ...
      days.forEach((day, dayIndex) => {
        ...
        dayHeader.appendChild(Dom.el('h2', 'text-2xl sm:text-3xl font-bold text-white whitespace-nowrap', TimeUtils.formatDayHeading(day.date)));
        dayHeader.appendChild(Dom.el('div', 'hr-gradient flex-1 mt-1'));
        dayHeader.appendChild(Dom.el('span', 'text-xs uppercase tracking-widest text-pnsqc-gold font-semibold whitespace-nowrap', `Day ${dayIndex + 1}`));
        daySection.appendChild(dayHeader);

        const timeline = Dom.el('div', 'space-y-6');
        const sessions = Array.isArray(day.sessions) ? day.sessions.slice() : [];
        sessions.sort((a, b) => String(a?.start || '99:99').localeCompare(String(b?.start || '99:99')));

        if (sessions.length === 0) {
          const noSessions = Dom.el('div', 'p-4 border border-white/10 rounded-lg text-pnsqc-slate');
          ...
          grouped.forEach((group) => {
            ...
            if (group.length > 1) {
              ...
              singleWrapper.appendChild(this.buildSessionCard({
                session: group[0],
                dayIso: day.date,
                timeFormat,
                eventTimeZone,
  ...
  class AtAGlanceApp {
    ...
    async init() {
      try {
        ...
        const submissionIds = this.renderer.collectSubmissionIds(data);
        await this.client.fetchSubmissions(submissionIds, this.renderer.submissionDetails);
    ...
    renderError() {
      ...
      if (root) {
        ...
        card.appendChild(Dom.el('p', 'text-red-100 text-lg', 'The live schedule is temporarily unavailable. Please check back shortly.'));

...
5. **Dist assembly** (`node build.mjs dist`) — copies only publishable files into `dist/`, excluding source files, build tooling, and `node_modules/`
...
The repo root contains source files (`build.mjs`, `package.json`, `content/`, `_partials/`, etc.) that should not be served publicly. Deploying the repo root as the web root would expose build tooling, `.git/` history, and `node_modules/` to anyone who guesses the URL.

Running `npm run build` (or `npm run build:dist`) assembles a clean `dist/` directory containing only the files visitors need: HTML pages, compiled CSS, images, `sitemap.xml`, and `robots.txt`. Point your static host's publish directory at `dist/` instead of the repo root.

`dist/` is git-ignored and rebuilt from scratch on every build.
...
└── dist/                            # publish directory (git-ignored, built)
```
